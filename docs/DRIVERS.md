...
- ✅ **Synchronous Operations**: All operations are synchronous for maximum performance
- ✅ **Complete API Support**: Implements all optional methods
...
export default defineDriver<void, Map<string, any>>(() => {
  ...
  return {
    ...
    getKeys() { return [...data.keys()] },
    clear() { data.clear() },
    dispose() { data.clear() }

    // Raw operations (no serialization needed)
    getItemRaw(key) { return data.get(key) ?? null },
    setItemRaw(key, value) { data.set(key, value) }
  }
})
```
...
// Performance testing
console.time('memory-ops')
for (let i = 0; i < 100000; i++) {
  await storage.setItem(`key-${i}`, { value: i })
}
console.timeEnd('memory-ops') // ~100ms for 100k operations
```
...
- ❌ **Performance**: Slower than memory due to I/O operations
...
export default defineDriver((options: FSStorageOptions = {}) => {
  ...
  const ignore = anymatch(options.ignore || ["**/node_modules/**", "**/.git/**"])
  ...
  return {
    ...
    async getMeta(key) {
      const { atime, mtime, size, birthtime, ctime } =
        await fsp.stat(resolvePath(key)).catch(() => ({} as Stats))
...
const flushQueue = async () => {
  ...
  try {
    ...
    const operations = [...context.queue.values()]
      .sort((a, b) => a.timestamp - b.timestamp)

    // Group by operation type
    const setOps = operations.filter(op => op.type === 'set' && op.value !== undefined)
    const removeOps = operations.filter(op => op.type === 'remove')

    // Use batch operations if available
    if (driver.setItems && setOps.length > 1 && setOps.every(op => !op.isRaw)) {
      await driver.setItems(setOps.map(op => ({
        key: op.key,
        value: op.value,
        options: op.options
      })))
      ...
      await Promise.all(setOps.map(op => {
    ...
    // Handle removals
    await Promise.all(removeOps.map(op =>
...
```typescript
import { defineDriver, createError } from 'electron-async-storage/drivers/utils'
  ...
  return {
    ...
    async getItem(key, opts) {
      ...
      if (value !== null) {
        ...
        cache.setItem?.(cacheKey, value, { ...opts, ttl: cacheTTL })
          .catch(console.error)
      }
...
async function benchmarkDriver(storage: Storage, operations = 10000) {
  ...
  console.log(`Write: ${writeTime.toFixed(2)}ms (${(operations / writeTime * 1000).toFixed(0)} ops/sec)`)
  ...
  console.log(`Read: ${readTime.toFixed(2)}ms (${(operations / readTime * 1000).toFixed(0)} ops/sec)`)
  ...
  console.log(`Batch Read: ${batchTime.toFixed(2)}ms (${(operations / batchTime * 1000).toFixed(0)} ops/sec)`)
}
```
